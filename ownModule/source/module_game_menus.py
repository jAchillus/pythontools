...
#This needs to be the first window!!!
  ("start_game_options", 0,
    "Welcome to Custom Commander, a mod for Mount and Blade: Warband. Please select your game options.\
 These options cannot be changed after the game has begun!\
 ^^Option 1: Assemble NPCs ({s1})^If enabled, all 16 NPCs will be assembled to the tavern of the first encountered town.\
 ^^Option 2: Add bandit heroes ({s2})^If enabled, 6 bandit heroes will appear in the wildness according to your character's level.\
 The bandit parties will come to accompany them. They become much united, not like a sheet of loose sand.\
 ^^Option 3: Skill penalty by encumbrance ({s3})^If enabled, the encumbrance will give skill penalty on character's power draw, riding and horse archery.\
 Raise character's strength and agility can reduce the penalty by encumbrance.",
    "none", 
    [
      (try_begin),
        (eq, "$g_gether_npcs", 1),
      ...
      (try_begin),
        ...
        (str_store_string, s2, "@Enabled"),
      (else_try),
        (str_store_string, s2, "@Disabled"),
      (try_end),
      (try_begin),
        (eq, "$g_encumbrance_penalty", 1),
        (str_store_string, s3, "@Enabled"),
      (else_try),
        (str_store_string, s3, "@Disabled"),
      (try_end),
    ],
    [
      ("option_1", 
        ...
        "{s1} the Option 1.",
        [
          (val_add, "$g_gether_npcs", 1),
          (val_mod, "$g_gether_npcs", 2),
      ...
      ("option_2", 
        [
          (try_begin),
            (eq, "$g_add_bandit_heroes", 1),
            (str_store_string, s2, "@Disable"),
          (else_try),
            (str_store_string, s2, "@Enable"),
          (try_end),
        ],
        "{s2} the Option 2.",
        [
          (val_add, "$g_add_bandit_heroes", 1),
          (val_mod, "$g_add_bandit_heroes", 2),
          (jump_to_menu,"mnu_start_game_options"),
        ]),
      ("option_3", 
        [
          (try_begin),
            (eq, "$g_encumbrance_penalty", 1),
            (str_store_string, s3, "@Disable"),
          (else_try),
            (str_store_string, s3, "@Enable"),
          (try_end),
        ],
        "{s3} the Option 3.",
        [
          (val_add, "$g_encumbrance_penalty", 1),
          (val_mod, "$g_encumbrance_penalty", 2),
          (jump_to_menu,"mnu_start_game_options"),
        ]),
      ...
      ("continue", [],
        ...
        [
          (try_begin),
            (eq, "$g_gether_npcs", 1),
            (set_show_messages, 0),
            (troop_add_gold, "trp_player", 3300),
            (set_show_messages, 1),
          (try_end),
...
##      ]),
  ...
  ("reports",0,
   "Character Renown: {reg5}^Honor Rating: {reg6}^Party Morale: {reg8}^Party Size Limit: {reg7}^^{reg9?Next pay time:^{s1}:}^{s5}", ## CC
   "none",
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (try_for_range, ":pt_no", bandit_party_template_begin, bandit_party_template_end),
      (try_for_parties, ":party_no"),
        ...
        (str_store_troop_name_plural, s4, ":leader"),
        (str_store_string, s5, "@{s5}^{s3} of {s4} is wandering around {s2}."),
      (try_end),
    (try_end),
    ## CC
    ...
    [
      ...
      # ("action_view_upgrade_trees",[],"View upgrade trees.",
       # [
        # (assign, "$temp_2", 0),
        # (start_presentation, "prsnt_upgrade_trees"),
        # ]
       # ),
...
## CC
  ...
  ("morale_report",0,
   ...
   [
    ...
    (else_try),
      (str_store_string, s8, "@ "),
    (try_end),
    (str_store_string, s1, "@Current party morale is {reg5}.^Current party morale modifiers are:^^Base morale:  +50^Party size: {s2}{reg1}^Leadership: {s3}{reg2}^Player's level: +{reg9}^Food variety: {s4}{reg3}{s5}{s6}^Recent events: {s7}{reg4}^TOTAL:  {reg5}^^Morale value to change: {s8}{reg8}^Prospective party morale: {reg10}^^^"),
    ## CC
     ...
     (try_for_range, ":kingdom_no", npc_kingdoms_begin, npc_kingdoms_end),
       ...
       (str_store_string, s1, "@{s1}Extra morale for {s9} troops : {reg6}^"), ## CC
     (try_end),        
    ],
    [
      ("continue",[],"Continue...",
  ...
  ("party_size_report",0,
   ...
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (assign, reg3, ":renown"),
    (str_store_string, s1, "@Current party size limit is {reg5}.^Current party size modifiers are:^^Base size:  +30^Player's level: +{reg7}^Leadership: {s2}{reg1}^Charisma: {s3}{reg2}^Renown: {s4}{reg3}^Castles: {s6}{reg6}^TOTAL:  {reg5}"), ## CC
    ],
    [
      ("continue",[],"Continue...",
       [(jump_to_menu, "mnu_reports"),
    ...
    ("camp_recruit_prisoners",
     ...
     [
      ...
      (else_try),
        ...
        (store_mul, ":recruit_chance", ":persuasion_level", 3),
        (val_add, ":recruit_chance", 20),
        (party_clear, "p_temp_party"), # p_temp_party do as a list of recruited prisoners
        (party_get_num_prisoner_stacks, ":num_stacks", "p_main_party"),
        (try_for_range, ":cur_stack", 0, ":num_stacks"),
          (party_prisoner_stack_get_troop_id, ":cur_troop_id", "p_main_party", ":cur_stack"),
          (neg|troop_is_hero, ":cur_troop_id"),
          (party_prisoner_stack_get_size, ":stack_size", "p_main_party", ":cur_stack"),
          (assign, ":rand_limit", 100),
          (try_begin),
            (le, ":stack_size", 5),
            (assign, ":rand_limit", 50),
          (try_end),
          (assign, ":num_recruit", 0),
          (try_for_range, ":unused", 0, ":stack_size"),
            (store_random_in_range, ":rand_no", 0, ":rand_limit"),
            (lt, ":rand_no", ":recruit_chance"),
            (val_add, ":num_recruit", 1),
          (try_end),
          (gt, ":num_recruit", 0),
          (party_add_members, "p_temp_party", ":cur_troop_id", ":num_recruit"),
        (try_end),
        (party_get_num_companions, ":total_recruit", "p_temp_party"),
        (try_begin),
          (le, ":total_recruit", 0), # is empty
          (jump_to_menu, "mnu_camp_no_prisoners_join"),
        (else_try),
          (jump_to_menu, "mnu_camp_recruit_prisoners_new"),
          (assign, "$temp", -1), # cur_stack_no of p_temp_party, begin from -1
          (party_clear, "p_dhorak_keep"), # p_dhorak_keep to store real recruited prisoners
        (try_end),
      (try_end),
      ## CC
      ],
     ),
    # ("camp_sort_player_inventory",[],"Sort your inventory.",
     # [
      # (assign, "$temp_2", -1),
    ...
    ("camp_manage_inventory",
      ...
      ## CC
        ...
        (try_for_range, ":i_stack", 0, ":num_stacks"),
          ...
          (try_for_range, ":cur_troop", pool_troops_begin, pool_troops_end),
            (call_script, "script_transfer_inventory", ":this_hero", ":cur_troop"),
          (try_end),
        (try_end),
      ...
      ("camp_action",[],"More and more actions.",
...
#### export/import NPCs end ####
  ...
  ("camp_recruit_prisoners_new",0,
   ...
   [
      ...
      (val_add, "$temp", 1), # jump to next stack of p_temp_party
      (party_get_num_companion_stacks, ":num_stacks", "p_temp_party"),
      (party_get_free_companions_capacity, ":capacity", "p_main_party"),
      (try_begin),
        (this_or_next|le, ":capacity", 0), # no rooms
        (ge, "$temp", ":num_stacks"), # out of p_temp_party stacks
        (jump_to_menu, "mnu_camp"),
      (else_try),
        (party_stack_get_troop_id, ":cur_prisoner", "p_temp_party", "$temp"),
        (gt, ":cur_prisoner", 0),
        # (party_get_num_companion_stacks, ":main_party_stacks", "p_main_party"),
        (try_begin), # out of 32 stacks
          # (ge, ":main_party_stacks", 32),
          # (party_count_companions_of_type, ":main_party_this_troop", "p_main_party", ":cur_prisoner"),
          # (le, ":main_party_this_troop", 0),
          # (jump_to_menu, "mnu_camp"),
        # (else_try),
          (party_count_companions_of_type, ":prisoner_amount", "p_temp_party", ":cur_prisoner"),
          (party_get_free_companions_capacity, ":capacity", "p_main_party"),
          (val_min, ":prisoner_amount", ":capacity"),
          (assign, reg1, ":prisoner_amount"),
          (gt, ":prisoner_amount", 0),
          (try_begin),
            (gt, ":prisoner_amount", 1),
            (assign, reg2, 1),
          (else_try),
            (assign, reg2, 0),
          (try_end),
          (str_store_troop_name_by_count, s1, ":cur_prisoner", ":prisoner_amount"),
          (str_store_string, s18, "@{reg1} {s1} {reg2?accept:accepts} the offer."),
          (assign, "$g_prisoner_recruit_troop_id", ":cur_prisoner"),
          (assign, "$g_prisoner_recruit_size", ":prisoner_amount"),
        (try_end),
      (try_end),
    ],
    [
      ("camp_recruit_prisoners_accept",[],"Take them.",
       [
        (party_remove_prisoners, "p_main_party", "$g_prisoner_recruit_troop_id", "$g_prisoner_recruit_size"),
        (party_add_members, "p_main_party", "$g_prisoner_recruit_troop_id", "$g_prisoner_recruit_size"),
        (store_mul, ":morale_change", -3, "$g_prisoner_recruit_size"),
        (call_script, "script_change_player_party_morale", ":morale_change"),
        # p_dhorak_keep to store real recruited prisoners
        (party_add_members, "p_dhorak_keep", "$g_prisoner_recruit_troop_id", "$g_prisoner_recruit_size"),
        # next stack of p_temp_party
        # (val_add, "$temp", 1), 
        # (party_get_num_companion_stacks, ":num_stacks", "p_temp_party"),
        # (party_get_free_companions_capacity, ":capacity", "p_main_party"),
        # (try_begin), # no rooms
      ...
      ("camp_recruit_prisoners_reject",[],"Reject them.",
       [
        # next stack of p_temp_party
        # (val_add, "$temp", 1), 
        # (party_get_num_companion_stacks, ":num_stacks", "p_temp_party"),
  ...
  ("camp_no_prisoners_join",0,
   "You offer your prisoners freedom if they agree to join you as soldiers. No one accepts the offer.",
   "none",
   [],
    [
      ("continue",[],"Continue...",
      ...
      ("exchange_village_accept",[],"Accept.",
       [
        ...
        (try_begin), # skip if trp_player is faction leader
          ...
          (faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
        (else_try),
          (call_script, "script_troop_change_relation_with_troop", ":village_lord", ":village_lord_2", 5),
        (try_end),
        (val_add, "$temp_3", 1), # begin from next village
        (jump_to_menu, "mnu_exchange_village"),
        ]
 ...
 all your money, and ride off into the sunset....",
  ...
  ("cattle_herd_kill",0,
   ...
   [(party_get_num_companions, reg5, "$g_encountered_party")],
    [
      ...
      ("cattle_kill_5",[(ge, reg5, 20),],"Twenty.",
  ...
  (
    "simple_encounter",mnf_scale_picture|mnf_enable_hot_keys, ## CC
    "{s2} You have {reg10} troops fit for battle against their {reg11}.^^P, C , I keys are enabled here.",
    "none",
    [      
...
#       (call_script, "script_music_set_situation_with_culture", mtf_sit_victorious),
     ...
     (else_try),
       ...
       (troop_get_type, ":is_female", "$g_player_troop"), ## CC
       (try_begin),
         (eq, ":is_female", 1),
...
##            (eq, ":random_attribute", ca_intelligence),
  ...
  (
    ...
    "You are helping the {s2} against the {s1}. You have {reg10} troops fit for battle against the enemy's {reg11}.^^P, C , I keys are enabled here.",
    "none",
    [                
...
##  ),
 ...
 where you and your men are welcomed.^^P, C , I keys are enabled here.",
    "none",
    [        
        (str_store_party_name, s1, "$g_encountered_party"),
        (str_store_party_name, s2, "$g_encountered_party_2"),
...
#                                                       (set_jump_mission,"mt_town_default"),
 ...
 and the guard sergeant informs you that his men will fire if you attempt to come any closer.",
  ...
  (
   ...
   (
    "castle_besiege",mnf_scale_picture|mnf_enable_hot_keys, ## CC
    "You are laying siege to {s1}. {s2} {s3}^^P, C , I keys are enabled here.",
    "none",
    [
        (troop_get_type, ":is_female", "$g_player_troop"), ## CC
        (try_begin),
...
##  ),
  ...
  (
    ...
    "{s1} is launching an assault against the walls of {s2}. You have {reg10} troops fit for battle against the enemy's {reg11}. You decide to...^^P, C , I keys are enabled here.",
    "none",
    [
        (select_enemy,1),
        (assign, "$g_enemy_party", "$g_encountered_party_2"),
  ...
  (
    ...
    [
        ...
        (try_begin),
		  ...
		  (try_begin),
      ## CC
      (this_or_next|is_between, "$current_town", "p_village_138", villages_end),
      ## CC
			(is_between, "$current_town", "p_village_91", "p_village_111"),
      ...
      ("village_buy_food",[(party_slot_eq, "$current_town", slot_village_state, 0),
       ...
       [
         (try_begin),
           (call_script, "script_cf_enter_center_location_bandit_check"),
         (else_try),
           (jump_to_menu,"mnu_village_trade"),
  ...
  (
    ...
    [
      ("auto_sell", [],
       ...
       [
          (assign, "$g_next_menu", "mnu_village_trade"),
          (jump_to_menu,"mnu_trade_auto_sell_begin"),
        ]),
      ("auto_buy_food", [],
       "Buy food automaticly.",
       [
          (assign, "$g_next_menu", "mnu_village_trade"),
          (jump_to_menu,"mnu_trade_auto_buy_food_begin"),
        ]),
      ("trade_with_the_peasants",[],
       "Trade with the peasants.",
       [
           (party_get_slot, ":merchant_troop", "$current_town", slot_town_elder),
           (change_screen_trade, ":merchant_troop"),
        ]),
      ("back_to_village_menu",[],"Head back.",
       [
          (party_get_slot, ":merchant_troop", "$current_town", slot_town_elder),
          (troop_sort_inventory, ":merchant_troop"), 
          (call_script, "script_sort_merchant_inventory_by_type", ":merchant_troop"),
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
  ...
  (
    ...
    [(party_get_slot, ":volunteer_troop", "$current_town", slot_center_volunteer_troop_type),
     ...
     (store_div, ":join_cost", reg0, 2),
     ## CC
     (store_div, ":gold_capacity", ":gold", ":join_cost"),## CC
     ...
     (else_try),
       (store_mul, reg6, ":volunteer_amount", ":join_cost"), ## CC
  ...
  (
    ...
    [#(assign, ":num_improvements", 0),
     ...
     # (try_for_range, ":improvement_no", ":begin", ":end"),
       # (party_slot_eq, "$g_encountered_party", ":improvement_no", 1), ## CC
       # (val_add,  ":num_improvements", 1),
       # (call_script, "script_get_improvement_details", ":improvement_no", "$g_encountered_party"), ## CC
       # (try_begin),
     ...
     (try_for_range, ":improvement_no", ":begin", ":end"),
       ...
       (call_script, "script_get_improvement_details", ":improvement_no", "$g_encountered_party"),
     ...
     (try_begin),
       ...
       (assign, ":recruiting_cost", reinforcement_cost_moderate),
       (assign, reg4, ":recruiting_cost"),
       (party_get_slot, reg1, "$g_encountered_party", slot_town_recruit_gold),
       (store_div, reg2, reg1, ":recruiting_cost"),
       (store_sub, reg3, reg2, 1),
       (store_troop_gold, reg5, "trp_player"),
       (str_store_string, s19, "@{s19}^You can lay some money to this {s17} for recruiting troops of {s2} automatically. You have {reg1} dinaers left here(can recruit for {reg2} {reg3?times:time}) and have {reg5} dinaers in your package."),
     (try_end),
     ...
     # (try_begin),
       ...
       # (call_script, "script_get_improvement_details", ":cur_improvement", "$g_encountered_party"),
       # (str_store_string, s7, s0),
       # (assign, reg6, 1),
       # (store_current_hours, ":cur_hours"),
    ...
    ## CC
      ...
      # ("castle_recruit_troops", 
        # [
          ...
          # (party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
        # ],
        # "Recruit some troops as garrison.", [(start_presentation, "prsnt_recruit_plan"),]),
        
      ("castle_recruit_increase_the_expense", 
        [
          (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
          (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
          (party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
        ],
        "Increase the expense of recruiting.", 
        [
          (assign, ":recruiting_cost", reinforcement_cost_moderate),
          (store_troop_gold, ":player_gold", "trp_player"),
          (ge, ":player_gold", ":recruiting_cost"),
          (troop_remove_gold, "trp_player", ":recruiting_cost"),
          (party_get_slot, ":recruit_gold", "$g_encountered_party", slot_town_recruit_gold),
          (val_add, ":recruit_gold", ":recruiting_cost"),
          (party_set_slot, "$g_encountered_party", slot_town_recruit_gold, ":recruit_gold"),
          (jump_to_menu, "mnu_center_manage"),
        ]),
        
      ("castle_recruit_decrease_the_expense", 
        [
          (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
          (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
          (party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
        ],
        "Decrease the expense of recruiting.", 
        [
          (assign, ":recruiting_cost", reinforcement_cost_moderate),
          (party_get_slot, ":recruit_gold", "$g_encountered_party", slot_town_recruit_gold),
          (ge, ":recruit_gold", ":recruiting_cost"),
          (val_sub, ":recruit_gold", ":recruiting_cost"),
          (party_set_slot, "$g_encountered_party", slot_town_recruit_gold, ":recruit_gold"),
          (troop_add_gold, "trp_player", ":recruiting_cost"),
          (jump_to_menu, "mnu_center_manage"),
        ]),
    ...
    ## CC
      ...
      ("center_build_barrack",[#(eq, reg6, 0),
                                      (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_barrack, 0),
                                       ],
       "Build a barrack.",[(assign, "$g_improvement_type", slot_center_has_barrack),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_smelter",[#(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_smelter, 0),
                                       ],
       "Build a smelter.",[(assign, "$g_improvement_type", slot_center_has_smelter),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_training_field",[#(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_training_field, 0),
                                       ],
       "Build a training field.",[(assign, "$g_improvement_type", slot_center_has_training_field),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ## CC      
...
#		(try_end),
    ...
    [
      ...
      ## CC
      ("join_tournament",  ## combine to "Enter the arena", still need this option here to keep the order of entries
        [
          (eq, 1, 0)
          # (neg|is_currently_night),
          # (party_slot_ge, "$current_town", slot_town_has_tournament, 1),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
            ...
            ## CC starting merchant
             ...
             #(else_try),
               ...
               ## CC
               # (gt, ":ransom_broker", 0),
               #(is_between, ":ransom_broker", ransom_brokers_begin, ransom_brokers_end),
               ## CC
...
## CC gambling begin
             (try_begin),
               ...
               (set_visitor, ":cur_entry", "trp_mystic_merchant"),
               (call_script, "script_refresh_mystic_merchant_items", "trp_mystic_merchant"),
               (val_add, ":cur_entry", 1),
             (try_end),
## CC gambling end
             # (try_begin),
               # (party_get_slot, ":ranger_master", "$current_town", slot_center_tavern_ranger_master),
               # (gt, ":ranger_master", 0),
               # (set_visitor, ":cur_entry", ":ranger_master"),
               # (val_add, ":cur_entry", 1),
             # (try_end),
             (try_begin),
...
#           (scene_slot_eq,  ":scene", slot_scene_visited, 1), #check if scene has been visited before to allow entry from menu. Otherwise scene will only be accessible from the town center.
       ...
       [
           (try_begin),
             (this_or_next|eq,"$all_doors_locked",1),
             (this_or_next|eq, "$sneaked_into_town", 1),
             (eq,"$town_nighttime",1),
             (display_message,"str_door_locked",0xFFFFAAAA),
           (else_try), # join the tournament
           ## CC
             (party_slot_ge, "$current_town", slot_town_has_tournament, 1),
             (call_script, "script_fill_tournament_participants_troop", "$current_town", 1),
             (assign, "$g_tournament_cur_tier", 0),
...
#			(troop_slot_eq, ":lady_guardian", slot_lord_granted_courtship_permission, -1),
	...
	(gt, "$love_interest_in_town", 0),
      ...
      ("trade_with_merchants",
       [
         ...
         "Go to the marketplace (Meet with the guild master).",
         [
           (try_begin),
             (call_script, "script_cf_enter_center_location_bandit_check"),
           (else_try),
    ...
    ## CC
      ...
      [
        ...
        (try_begin),
          (party_slot_eq, "$current_town", slot_town_lord, "trp_player"),
          (party_slot_eq, "$current_town", slot_center_has_barrack, 1),
          (val_mul, ":hours_needed", 3),
          (val_div, ":hours_needed", 4),
        (try_end),
      ...
      "Recruit troops{s1}.",
      [
        (assign, "$temp_2", 0), # show exchange screen only once
        (assign, "$temp_3", 1), # recruit times
        (try_begin),
          (party_slot_eq, "$current_town", slot_party_type, spt_town),
          (store_random_in_range, "$temp_3", 1, 3),
        (try_end),
        (jump_to_menu, "mnu_walled_center_recruit_troops"),
      ]),
    ## CC
      ...
      [
        ...
        (try_begin),
          ...
          (lt, ":gold", reg1),
          (assign, ":can_rest", 0),
          (disable_menu_option), ## CC
        (try_end),
        (ge, ":can_rest", 0), ## CC always available
      ],
...
## CC
      ("town_cheats_new", [(eq, "$cheat_mode", 1),],
       "Cheat menu.", 
       [(jump_to_menu,"mnu_town_menu_cheats"),]),
## CC
    ]
   ),
 ...
 In addition to honour, fame and glory, you earn a prize of {reg9} denars. {s8}",
  ...
  (
    ...
    [
      # ("bet_150_denars", [(store_troop_gold, ":gold", "trp_player"),
                          # (ge, ":gold", 150)
                          # ],
       # "150 denars.",
       # [
  ...
  (
    ...
    [
      ...
      [
        ...
        (ge, ":merchant_troop", 1),
        (try_begin),
          (this_or_next|eq, ":merchant_troop", "trp_town_2_weaponsmith"),
          (this_or_next|eq, ":merchant_troop", "trp_town_3_weaponsmith"),
          (this_or_next|eq, ":merchant_troop", "trp_town_8_weaponsmith"),
          (this_or_next|eq, ":merchant_troop", "trp_town_10_weaponsmith"),
          (this_or_next|eq, ":merchant_troop", "trp_town_20_weaponsmith"),
          (eq, ":merchant_troop", "trp_town_23_weaponsmith"),
          (str_store_troop_name, s1, ":merchant_troop"),
        (else_try),
          (str_store_string, s1, "@the arms merchant"),
        (try_end),
      ],
       "Trade with {s1}.",
       [
      ...
      ("back_to_town_menu",[],"Head back.",
       [
          ## CC
          (try_for_range, ":merchant_slot", slot_town_weaponsmith, slot_town_elder),
            (party_slot_ge, "$current_town", ":merchant_slot", 1),
            (party_get_slot, ":merchant_troop", "$current_town", ":merchant_slot"),
            (troop_sort_inventory, ":merchant_troop"), 
            (call_script, "script_sort_merchant_inventory_by_type", ":merchant_troop"),
          (try_end),
          ## CC
...
#      (else_try),
  ...
  (
    ...
    [
      ("continue",[],"Continue...",
       [
        (try_begin), # rebellion failed
          (is_between, "$g_notification_menu_var1", rebel_kingdoms_begin, rebel_kingdoms_end),
          (assign, ":rebel_faction", "$g_notification_menu_var1"),
          (store_add, ":original_faction", ":rebel_faction", num_kingdoms),
          (try_begin),
            (faction_slot_eq, ":original_faction", slot_faction_state, sfs_active),
            # faction_leader
            (faction_get_slot, ":rebel_faction_leader", ":rebel_faction", slot_faction_leader),
            (troop_set_slot, ":rebel_faction_leader", slot_troop_change_to_faction, "fac_commoners"),
            (troop_set_slot, ":rebel_faction_leader", slot_troop_occupation, slto_inactive_pretender),
            (faction_set_slot, ":rebel_faction", slot_faction_state, sfs_inactive),
            # marshall party
            (faction_get_slot, ":rebel_faction_marshall", ":rebel_faction", slot_faction_marshall),
            (try_begin),
              (ge, ":rebel_faction_marshall", 0),
              (troop_get_slot, ":rebel_faction_marshall_party", ":rebel_faction_marshall", slot_troop_leaded_party),
              (party_is_active, ":rebel_faction_marshall_party"),
              (party_set_marshall, ":rebel_faction_marshall_party", 0),
            (try_end),
            # lords & parties
            (try_for_range, ":cur_troop", active_npcs_begin, active_npcs_end),
              (troop_slot_eq, ":cur_troop", slot_troop_occupation, slto_kingdom_hero),
              (store_troop_faction, ":cur_faction", ":cur_troop"),
              (eq, ":cur_faction", ":rebel_faction"),
              (troop_set_faction, ":cur_troop", ":original_faction"),
              (call_script, "script_troop_set_title_according_to_faction", ":cur_troop", ":original_faction"),
            (try_end),
            (try_for_parties, ":cur_party"),
              (store_faction_of_party, ":cur_faction", ":cur_party"),
              (eq, ":cur_faction", ":rebel_faction"),
              (party_set_faction, ":cur_party", ":original_faction"),
            (try_end),
          (try_end),
          (assign, "$g_recalculate_ais", 1),
          (call_script, "script_update_all_notes"),
        (else_try), # rebellion success
          (is_between, "$g_notification_menu_var1", commom_kingdoms_begin, commom_kingdoms_end),
          (assign, ":original_faction", "$g_notification_menu_var1"),
          (store_sub, ":rebel_faction", ":original_faction", num_kingdoms),
          (try_begin),
            (faction_slot_eq, ":rebel_faction", slot_faction_state, sfs_active),
             # player
            (try_begin),
              (eq, "$players_kingdom", ":rebel_faction"),
              (assign, "$players_kingdom", ":original_faction"),
              (store_relation, ":relation", "fac_player_supporters_faction", ":rebel_faction"),
              (call_script, "script_set_player_relation_with_faction", ":original_faction", ":relation"),
              (try_begin),
                (troop_get_slot, ":spouse", "trp_player", slot_troop_spouse),
                (is_between, ":spouse", kingdom_ladies_begin, kingdom_ladies_end),
                (troop_set_faction, ":spouse", ":original_faction"),
              (try_end),
            (try_end),
            # faction_leader
            (faction_get_slot, ":old_leader", ":original_faction", slot_faction_leader),
            (faction_get_slot, ":new_leader", ":rebel_faction", slot_faction_leader),
            (faction_set_slot, ":rebel_faction", slot_faction_leader, ":old_leader"),
            (faction_set_slot, ":original_faction", slot_faction_leader, ":new_leader"),
            (troop_set_slot, ":old_leader", slot_troop_change_to_faction, "fac_commoners"),
            (troop_set_slot, ":old_leader", slot_troop_occupation, slto_inactive_pretender),
            (faction_set_slot, ":rebel_faction", slot_faction_state, sfs_inactive),
            # marshall party
            (faction_get_slot, ":old_marshall", ":original_faction", slot_faction_marshall),
            (try_begin),
              (ge, ":old_marshall", 0),
            ...
            (try_for_range, ":cur_kingdom", kingdoms_begin, kingdoms_end),
              ...
              (store_relation, ":reln", ":cur_kingdom", ":rebel_faction"),
              (set_relation, ":cur_kingdom", ":original_faction", ":reln"),
            (try_end),
            (call_script, "script_add_notification_menu", "mnu_notification_noble_rebels_switched_to_faction", ":original_faction", ":new_leader"),
            (faction_set_slot, ":original_faction", slot_faction_state, sfs_active),
            (faction_set_slot, ":original_faction", slot_faction_ai_state, sfai_default),
            (faction_set_slot, ":original_faction", slot_faction_ai_object, -1),
          (try_end),
          (assign, "$g_recalculate_ais", 1),
          (call_script, "script_update_all_notes"),
        (try_end),
        ...
        (try_for_range_backwards, ":troop_iterator", 0, ":num_stacks"),
          ...
          (party_remove_prisoners, "p_main_party", ":cur_troop_id", ":stack_size"),
        (try_end),
        
        # old code for pretenders
        (try_begin),
...
## CC
  ...
  (
    "notification_kingdom_rebellion",0,
    "Rebellion^^{s10} and another {reg1} lords have renounced their allegiance to the {s12} and hailed {s9} of {s11} as their leader.",
    "none",
    [
      (faction_get_slot, ":rebel_king", "$g_notification_menu_var1", slot_faction_leader),
      (str_store_troop_name, s9, ":rebel_king"),
      
      (assign, ":num_lords_sub_one", -1),
      (try_for_range, ":cur_troop", lords_begin, lords_end),
        (troop_slot_eq, ":cur_troop", slot_troop_temp_slot, "$g_notification_menu_var1"),
        (call_script, "script_change_troop_faction", ":cur_troop", "$g_notification_menu_var1"),
        (troop_set_slot, ":cur_troop", slot_troop_occupation, slto_kingdom_hero),
        ...
        (try_end),
      (try_end),
      (troop_set_slot, ":rebel_king", slot_troop_spawned_before, 0),
      (assign, reg1, ":num_lords_sub_one"),
      ...
      (str_store_faction_name, s11, "$g_notification_menu_var1"), # rebellion faction
      (str_store_faction_name, s12, "$g_notification_menu_var2"), # original faction
      ],
    [
      ("continue",[],"Continue...", 
        [
  ...
  (
    "notification_noble_rebels_switched_to_faction",0,
    "Rebellion Success^^ {s10} now has the sole claim to the title of {s11}, with {s12} as the new king.",
    "none",
    [
      (store_sub, ":rebel_kingdom", "$g_notification_menu_var1", num_kingdoms),
      (str_store_faction_name, s10, ":rebel_kingdom"),
      ...
      ("auto_loot_leave", [],
        ...
        [
          ...
          (try_for_range, ":i_stack", 0, ":num_stacks"),
            ...
            (troop_is_hero, ":stack_troop"),
            (neq, ":stack_troop", "trp_player"),
            (call_script, "script_transfer_inventory", "$pool_troop", ":stack_troop"),
            (try_for_range, ":cur_troop", pool_troops_begin, pool_troops_end),
              (call_script, "script_transfer_inventory", ":cur_troop", ":stack_troop"),
            (try_end),
          (try_end),
          (jump_to_menu, "$return_menu"),
        ]
...
#    ]),
  ...
  ("choose_options_2", 0,
    "Add bandit heroes into the game? If you choose yes, 6 bandit heroes will appear in the wildness according to your character's level.\
 The bandit parties will come to accompany them. They become much united, not like a sheet of loose sand.", "none", [],
    [
      ("bandit_heroes_yes", [], "Yes",
        [
          (assign, "$g_add_bandit_heroes", 1),
  ...
  ("choose_options_end", 0,
   "Game options selection complete.", "none", [],
    [
      ...
      ("go_back", [], "Go back", [(jump_to_menu,"mnu_choose_options_2"),]),
    ]),
## CC
  ...
  (
    "bandit_heroe_spawned",mnf_scale_picture,
    "{s1} of {s2} has staged a comeback.",
    "none",
    [
      
      (assign, ":pt_no", "$temp"),
      (party_template_get_slot, ":hero_id", ":pt_no", slot_party_template_hero_id),
      (party_template_get_slot, ":hero_name", ":pt_no", slot_party_template_hero_pre_name),
      
      (str_store_string, s1, ":hero_name"),
      (str_store_troop_name_plural, s2, ":hero_id"),
      
      (set_fixed_point_multiplier, 100),
      (position_set_x, pos0, 55),
      (position_set_y, pos0, 20),
      (position_set_z, pos0, 100),
      (set_game_menu_tableau_mesh, "tableau_troop_note_mesh", ":hero_id", pos0),
      ],
    [
      ("continue",[],"Continue...",
       [(change_screen_return),
  ...
  (
    "walled_center_recruit_troops",0,
    "The following soldiers want to join you.^^{s12}",
    "none",
    [
      (store_current_hours, ":hours"),
      (party_set_slot, "$current_town", slot_town_last_recruit_time, ":hours"),

      (call_script, "script_cf_generate_temp_party_for_player_center_recruiting", "$current_town"),
      (call_script, "script_print_party_to_s0", "p_temp_party"),
      (str_store_string_reg, s12, s0),
      (try_begin),
        (eq, "$temp_2", 1), # end
        (party_clear, "p_temp_party"),
    ...
    [
      ("recruit_to_your_party",[],"Recruit them to your party.",
       [
        (store_troop_gold, ":gold", "trp_player"),
        (ge, ":gold", reinforcement_cost_moderate),
        (troop_remove_gold, "trp_player", reinforcement_cost_moderate),
        (try_begin),
          # (call_script, "script_get_troop_kinds_of_parties", "p_main_party", "p_temp_party"),
          # (assign, ":troop_kinds", reg0),
          # (le, ":troop_kinds", 32),
          (party_get_free_companions_capacity, ":free_capacity", "p_main_party"),
          (party_get_num_companions, ":party_size", "p_temp_party"),
          (le, ":party_size", ":free_capacity"),
          (call_script, "script_party_add_party_companions", "p_main_party", "p_temp_party"),
          (party_clear, "p_temp_party"),
          (val_sub, "$temp_3", 1),
          (jump_to_menu, "mnu_walled_center_recruit_troops"),
        (else_try),
      ...
      ("recruit_to_cur_center",
        [
          (party_slot_eq, "$current_town", slot_town_lord, "trp_player"),
          # (call_script, "script_get_troop_kinds_of_parties", "$current_town", "p_temp_party"),
          # (assign, ":troop_kinds", reg0),
          # (le, ":troop_kinds", 32),
      ],"Recruit them to the garrison.",
       [
          (store_troop_gold, ":gold", "trp_player"),
          (ge, ":gold", reinforcement_cost_moderate),
          (troop_remove_gold, "trp_player", reinforcement_cost_moderate),
          (call_script, "script_party_add_party_companions","$current_town", "p_temp_party"),
          (party_clear, "p_temp_party"),
          (val_sub, "$temp_3", 1),
          (jump_to_menu, "mnu_walled_center_recruit_troops"),
        ]
       ),
      ("do_not_recruit", [],"Refuse them.",
       [
          (party_clear, "p_temp_party"),
          (val_sub, "$temp_3", 1),
          (jump_to_menu, "mnu_walled_center_recruit_troops"),
